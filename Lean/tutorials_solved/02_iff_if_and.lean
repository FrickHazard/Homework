...
begin
  ...
  { ring },
  rw key,
  apply mul_nonneg_of_nonpos_of_nonpos hc,
...
example (a b c : ℝ) (hc : c ≤ 0) (hab :  a ≤ b) : b*c ≤ a*c :=
begin
  have hab'': a - b ≤ 0 := sub_nonpos.mpr hab,
  have h₁: 0 ≤ (a - b)*c, { exact mul_nonneg_of_nonpos_of_nonpos hab'' hc } ,
...
begin
  rw ←sub_nonneg,
  calc 0 ≤ (a - b) * c : mul_nonneg_of_nonpos_of_nonpos _ hc
  ... = a * c - b * c : by ring,
...
begin
  ...
  intro adb,
  apply dvd_antisymm,
  exact (dvd_gcd_iff.mp (dvd_refl (a.gcd b))).left,
  exact dvd_gcd_iff.mpr ⟨dvd_refl a, adb⟩,
  intro gab,
  have h := dvd_gcd_iff.mp (dvd_refl (gcd a b)),
  have h1 := h.right,
  rwa gab at h1,
end
