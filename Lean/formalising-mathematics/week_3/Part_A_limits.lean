...
begin
  intros ε ε_pos,
  cases ha ε ε_pos with N₀ hN₀,
  use N₀,
  intros N geqN,
  calc
  -- apply N to i
    |(λ (i : ℕ), a i + c) N - (l + c)| = | a N + c - (l + c)| : by refl
    ... = | a N - l| : by ring
    ... < ε : hN₀ N (by linarith)
end
...
begin
  ...
  intro addConstLim,
  intros ε ε_pos,
  cases addConstLim ε ε_pos with N₀ hN₀,
  use N₀,
  intros N hN,
  calc
    |a N - l| = |a N - l + (c - c)| : by ring
    ... =  |a N + c  - (l + c)| : by ring
    ... < ε : hN₀ N (by linarith)
end
...
begin
  split,
  intro lim_a_to_l,
  convert (is_limit_add_const_iff (-l)).mp lim_a_to_l,
  ring,
  ring,
  intro lim_a_i_to_l,
  apply (is_limit_add_const_iff (-l)).mpr,
  convert lim_a_i_to_l,
  ring,
  ring
end
...
begin
  ...
  use (max N₁ N₂),
  intros n nGN,
  specialize hN₁ n (max_le_iff.mp nGN).1,
  specialize hN₂ n (max_le_iff.mp nGN).2,
  calc
  |(a + b) n - (l + m)| =  |a n + b n - (l + m)| : by refl
  ... = |(a n - l) + (b n - m)| : by ring
...
begin
  ...
  cases h (ε / |c|) (div_pos ε_pos (abs_pos.mpr hc)) with N₁ hN₁,
  use N₁,
  intros n hn,
  specialize hN₁ n hn,
  dsimp,
  calc
  |c * a n - c * l| = |(a n  - l) * c| : by ring
  ... = |a n - l| * |c|: abs_mul _ _
  ... < ε :  (lt_div_iff (abs_pos.mpr hc)).mp hN₁
end
...
have hh := abs_pos.mpr h,
rw abs_abs at hh,
 have gg:= mul_lt_mul hac (le_of_lt hb2) hh (le_of_lt he),
 ring at gg,
 ring,
 assumption,
end
...
begin
  ...
  cases hb 1 (by linarith) with N₂ hN₂,
  use (max N₁ N₂),
  intros n hn,
  specialize hN₁ n (max_le_iff.mp hn).1,
  specialize hN₂ n (max_le_iff.mp hn).2,
  ring at hN₁,
  ring at hN₂,
  have abs_bn_nonneg := abs_nonneg (b n),
  have abs_an_nonneg := abs_nonneg (a n),
  calc
    |(a * b) n | = |a n * b n | : by refl
    ... = |a n| * |b n| : abs_mul _ _
    ... < ε : mul_abs_lt_mul_abs (a n) (b n) ε hN₁ hN₂ ε_pos
end
#check (zero_add 0)
...
theorem is_limit_mul (a : ℕ → ℝ) (b : ℕ → ℝ) (l m : ℝ)
  ...
  is_limit (a * b) (l * m) :=
begin
  suffices : is_limit (λ (i : ℕ), (a i - l) * (b i - m) + (l * (b i - m)) + m * (a i - l)) 0,
  intros ε ε_pos,
  ...
  ring at hN,
  calc
    |a n * b n - l * m| = |b n * a n - m * l| : by ring
    ... < ε : hN,
  have a_sub_l_to_0 := (is_limit_iff_is_limit_sub_eq_zero a l).mp h1,
  have b_sub_m_to_0 := (is_limit_iff_is_limit_sub_eq_zero b m).mp h2,
  rw ←(zero_add (0 : ℝ)),
  apply is_limit_add,
  rw ←(zero_add (0 : ℝ)),
  apply is_limit_add,
...
begin
  ...
  have fact : 0 ≤ l - a n,
  linarith [h n],
  rw abs_of_nonneg fact,
  linarith [h n],
  apply this,
  assumption,
end

theorem is_limit_sub {a b : ℕ → ℝ} {l m : ℝ}
  (h1 : is_limit a l) (h2 : is_limit b m) :
  is_limit (a - b) (l - m) :=
begin
  intros ε ε_pos,
  ...
  |(a - b) n - (l - m)| =  |a n - b n - (l - m)| : by refl
  ... = |(a n - l) + (m - b n)| : by ring
  ... ≤ |a n - l| + |m - b n| : abs_add _ _
  ... = |a n - l| + |b n - m| : by rw (abs_sub m (b n))
  ... < ε : by linarith,
end
...
theorem is_limit_le_of_le (a : ℕ → ℝ) (b : ℕ → ℝ)
  ...
  (hle : ∀ n, a n ≤ b n) : l ≤ m :=
begin
  have fact1 : ∀ (n : ℕ), a n - b n ≤ 0,
    intros n,
    linarith [hle n],
  have ff := bounded_left (λ i, a i - b i) (l - m) 0 (is_limit_sub hl hm) fact1,
  linarith,
end
...
lemma tendsto_bounded_mul_zero {a : ℕ → ℝ} {b : ℕ → ℝ}
  ...
  : is_limit (a * b) 0 :=
begin
  cases hA with bnd hbnd,
  have lim_bnd_b := @is_limit_mul_const_left b 0 bnd hB,
  intros ε ε_pos,
  cases lim_bnd_b ε (by linarith) with N₂ hN₂,
  use N₂,
  intros n hn,
  specialize hN₂ n hn,
  rw [mul_zero, sub_zero] at hN₂,
  rw sub_zero,
  have solution : |a n * b n| ≤ |bnd * b n|, {
    rw [abs_mul, abs_mul],
    by_cases H : |b n| = 0,
    rw H,
    linarith,
    let pos := abs_pos.mpr H,
    rw abs_abs at pos,
    apply mul_le_mul,
    linarith [(hbnd n), le_abs_self bnd],
    linarith,
    linarith,
    linarith [abs_nonneg bnd],
  },
  calc
  |(a * b) n| = |a n * b n| : by refl
  ... ≤ |bnd * b n| : solution
