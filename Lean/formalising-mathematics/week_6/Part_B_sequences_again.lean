...
begin
  intros yz yz_in𝓝,
  rcases mem_nhds_prod_iff.mp yz_in𝓝 with ⟨u, u_in𝓝y, v, v_in𝓝z, h⟩,
  have u_in_mapf := (hf u_in𝓝y),
  have v_in_mapg := (hg v_in𝓝z),
  rw mem_map at *,

  -- could rw this without applies
  apply mem_sets_of_superset (inter_mem_sets u_in_mapf v_in_mapg),
...
begin
  ...
  { refl },
  rw ←h1,
  have prdF := tendsto.prod_mk_nhds hf hg,
  rw ← hf2 at prdF,
  apply tendsto.comp hcontinuous,
  exact prdF,
end
