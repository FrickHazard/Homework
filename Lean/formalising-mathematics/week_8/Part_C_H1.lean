...
{ carrier := { f : G → M | ∀ (g h : G), f (g * h) = f g + g • f h },
  ...
  add_mem' := begin
    intros mp1 mp2 a b g h,
    specialize a g h,
    specialize b g h,
    simp,
    -- help simp, could be shorter prob
    conv {
      to_rhs,
      rw ←add_assoc,
      rw add_assoc (mp1 g) (mp2 g) (g • mp1 h),
      rw add_comm (mp2 g) (g • mp1 h),
      rw ←add_assoc,
      rw ← a,
      rw add_assoc,
      rw ←b,
...
{ to_fun := λ g, a g + b g,
 ...
 := begin
   ...
   conv {
     to_rhs,
     rw add_assoc,
     rw add_comm (b g1) (g1 • a g2 + g1 • b g2),
     rw ←add_assoc,
     rw ←add_assoc,
     rw add_assoc,
     rw add_comm (g1 • b g2)  (b g1) ,
     rw ← add_assoc,
   },
   rw ←add_assoc,
  end }
