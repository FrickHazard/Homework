...
begin
  intros hgm1 hgm2 h,
  have hh := ( H0.ext_iff _ _).mp h,
  rw [ H0.coe_apply hgm1 φ, H0.coe_apply hgm2 φ] at hh,
  apply ( H0.ext_iff _ _).mpr,
  exact hφ hh,
end
...
begin
  ext hgn,
  --rcases h with ⟨exct, inj, surj⟩,
  split,
  {
    intro hgn_in_range,
    ext,
    simp,
    apply (h.exact_set hgn).mp,
    cases hgn_in_range with hgm hhgm,
    use hgm,
    rw ← (H0.coe_apply hgm φ ),
    rw hhgm,
  },
  {
    intro hgn_in_ker,
    simp,
    have: ψ ↑hgn = 0, {
     rw ← H0.coe_apply hgn ψ,
     apply (H0.ext_iff ((ψ.H0) hgn) 0).mp,
     exact hgn_in_ker,
    },
    cases ((h.exact_set hgn).mpr this) with m hm,
    use m,
    intro g,
    apply h.2.1,
    simpa [hm] using hgn.spec g,
    ext,
    exact hm,
  },
end
