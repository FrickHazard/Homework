...
begin
  ...
  intro ST_in_F,
  constructor,
  { exact mem_sets_of_superset ST_in_F (inter_subset_left S T) },
  { exact mem_sets_of_superset ST_in_F (inter_subset_right S T) },
  rintros ⟨SinF, TinF⟩,
  exact inter_mem_sets SinF TinF,
end
...
{ sets := {S : set α | X ⊆ S},
  ...
  sets_of_superset := begin
    intros set1 set2 set1inXofS set1sbset2,
    exact subset.trans set1inXofS set1sbset2,
  end,
  inter_sets := begin
    intros set1 set2 set1InX set2InX,
    exact subset_inter set1InX set2InX,
  end }
...
begin
  ...
  {
    intro filS_le_filT,
    exact mem_principal_sets.mp (filS_le_filT (mem_principal_self T)),
    -- TODO question about inference, bottom line requires *T*
    -- rw le_def at filS_le_filT,
    -- exact mem_principal_sets.mp (filS_le_filT T (mem_principal_self T)),
  },
  {
    intros S_sbst_T seta setainPriFilT,
    exact mem_principal_sets.mpr (subset.trans S_sbst_T (mem_principal_sets.mp setainPriFilT)),
  },
end
...
begin
  split,{
    intro f_le_ps,
    exact f_le_ps (mem_principal_self S),
  }, {
    intros sinF seta setain𝓟S,
    exact mem_sets_of_superset sinF (mem_principal_sets.mp setain𝓟S),
  },
end
...
{ sets := {X : set L | ∃ x : L, ∀ y, x ≤ y → y ∈ X},
  ...
  sets_of_superset := begin
    intros set1 set2 set1InAt_top set1_sub_set2,
    cases set1InAt_top with l l_order,
    use l,
    intros y h,
    exact set1_sub_set2 (l_order y h),
  end,
  inter_sets := begin
    intros set1 set2 set1InAt_top set2InAt_top,
    cases set1InAt_top with y₁ hy₁,
    cases set2InAt_top with y₂ hy₂,
    use max y₁ y₂,
    intros l h,
    constructor,
    { exact hy₁ l (le_trans (le_max_left y₁ y₂) h) },
    { exact hy₂ l (le_trans (le_max_right y₁ y₂) h)},
  end }
...
{ sets := { S : set α | (Sᶜ).finite },
  univ_sets := begin
    have:= finite_empty,
    rwa ←compl_univ at this,
  end,
  sets_of_superset := begin
    intros set1 set2 set1Confinite set1_sub_set2,
    exact set1Confinite.subset (compl_subset_compl.mpr set1_sub_set2),
  end,
  inter_sets := begin
    intros set1 set2 set1Prop set2Prop,
    have:= set1Prop.union set2Prop,
    rwa ←compl_inter at this,
  end }
