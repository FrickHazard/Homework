...
begin
  split,
  intro h,
  have invinv := (inv_mem H h),
  rw xena.group.inv_inv at invinv,
  assumption,
  intro h,
  have invinv := (inv_mem H h),
  assumption,
...
begin
  have x_inv_in_H : x⁻¹ ∈ H,
    simp [hx],
  have x_inv_x_y_in_H: x⁻¹ * (x * y) ∈ H,
    exact mul_mem H x_inv_in_H hxy,
  simp at x_inv_x_y_in_H,
  assumption
end
...
{ carrier := H.carrier ∩ K.carrier, -- the carrier is the intersection
  ...
  begin
    intros x xinHintK,
    exact ⟨(inv_mem H) xinHintK.1, (inv_mem K) xinHintK.2⟩,
  end }
...
begin
  ...
  intros subG s_subst_subG,
  exact s_subst_subG ginS,
end
...
begin
  intros g ginS,
  have aa := subset.trans hST (subset_closure T),
  have ff := mem_closure_iff.mp ginS (closure T) aa,
  assumption,
end
...
begin
  split,
  intros closure_S_le_H s sinS,
  exact closure_S_le_H ((subset_closure S) sinS),
  intros sinH g ginS_closure,
  exact mem_closure_iff.mp ginS_closure H sinH,
end
...
begin
  apply le_antisymm,
  intros g ginClosCarrier,
  apply mem_closure_iff.mpr,
  intros H ssubstH,
  apply ssubstH,
  assumption,

  intros g ginClosClosS,
  apply (closure_le (closure S).carrier (closure S)).mpr,
  intros g ginC,
  exact ginC,
  assumption,
end
...
begin
  ...
  split,
  intros xinClosH,
  apply mem_closure_iff.mp xinClosH,
  refl,
  intros xinH,
  exact subset_closure H.carrier xinH,
end
